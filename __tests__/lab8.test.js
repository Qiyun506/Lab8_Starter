describe('Basic user flow for Website', () => {
  ...
  it('Clicking the "Add to Cart" button should change button text', async () => {
    ...
    let SR = await item.getProperty('shadowRoot');
    let button =await SR.$('button');
    // Once you have the button, you can click it and check the innerText property of the button.
    await button.click();
    let innerTxt = await button.getProperty('innerText');
    innerTxt = await innerTxt.jsonValue();
    await button.click();
    expect(innerTxt).toBe('Remove from Cart');
  ...
  it('Checking number of items in cart on screen', async () => {
    ...
    for(let i =0; i < item.length; i++){
      ...
      let SR = await singleItem.getProperty('shadowRoot');
      let button =await SR.$('button');
      await button.click();
    }
    let cCount = await page.$('#cart-count');
    let innerTxt = await cCount.getProperty('innerText');
    number = await innerTxt.jsonValue();
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    for(let i =0; i < item.length; i++){
      ...
      let innerTxt = await button.getProperty('innerText');
      innerTxt = await innerTxt.jsonValue();
      expect(innerTxt).toBe("Add to Cart");
    }
